...
export default function CreateReceitaScreen() {
  ...
  // Conta quantas tags estão selecionadas
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/receitas`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ ...form, foto_url }),
      });
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || "Erro ao salvar receita."); }
  ...
  return (
    <View style={styles.outer}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Cadastro{"\n"}de receita</Text>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.fieldWrap}>
          ...
          <TextInput
            ...
            placeholder="Ex: Wrap Integral de Frango"
        ...
        {/* Botão */}
        <TouchableOpacity style={[styles.btn, loading && { opacity: 0.7 }]} onPress={handleSalvar} disabled={loading} activeOpacity={0.85}>
