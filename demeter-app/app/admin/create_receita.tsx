...
// @ts-ignore: expo-image-picker may not have type declarations in this environment
...
// ─── Tela principal ──────────────────────────────────────────────────────────
export default function AdminReceitaScreen() {
  ...
  const uploadFoto = async (uri: string): Promise<string> => {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || "Erro no upload da foto.");
    }
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (fotoUri) {
        foto_url = await uploadFoto(fotoUri);
      }

      const response = await fetch(`${API_URL}/api/admin/receitas`, {
        method: "POST",
        headers: {
